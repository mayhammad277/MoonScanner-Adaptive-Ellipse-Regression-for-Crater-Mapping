...
from train import SwinCrater320
...
class SizePriorityLoss(nn.Module):
    ...
    def forward(self, preds, target):
        ...
        return (10.0 * hm_loss) + (40.0 * a_loss) + (10.0 * o_loss)
...
def run_inference(model_path, test_dir, output_csv):
    ...
    paths = list(Path(test_dir).rglob("*.png"))
    for p in tqdm(paths):
        ...
        hmax = torch.nn.functional.max_pool2d(hm.unsqueeze(0).unsqueeze(0), 3, stride=1, padding=1).squeeze()
        hm = hm * (hmax == hm).float()
        
        scores, idxs = torch.topk(hm.view(-1), k=50)
        for s, idx in zip(scores, idxs):
            if s < 0.50: continue # High confidence threshold for accuracy
            iy, ix = np.unravel_index(idx.cpu().numpy(), (320, 320))
            
            # RECOVERY LOGIC (Matches Train)
    ...
    cols = ['ellipseCenterX(px)', 'ellipseCenterY(px)', 'ellipseSemimajor(px)', 'ellipseSemiminor(px)', 'ellipseRotation(deg)', 'inputImage', 'crater_classification']
    pd.DataFrame(results, columns=cols).to_csv(output_csv, index=False)

if __name__ == "__main__":
    run_inference("best_size_model.pth", "test_data_dir", "submission.csv")
