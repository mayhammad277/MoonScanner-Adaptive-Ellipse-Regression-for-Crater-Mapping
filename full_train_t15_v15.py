...
def main():
    ...
    parser.add_argument("--data_dir", required=True); parser.add_argument("--gt_csv", required=True)
    ...
    # Batch size of 4 is usually required for 640x640 on most GPUs
    train_loader = DataLoader(CraterDataset640(df, args.data_dir, t_imgs), batch_size=32, shuffle=True)
    val_loader = DataLoader(CraterDataset640(df, args.data_dir, v_imgs), batch_size=32, shuffle=False)
    model = SwinCrater640().to(DEVICE)
    optimizer = optim.AdamW(model.parameters(), lr=8e-5) # Slightly lower LR for stability
    ...
    for ep in range(70):
        ...
        print(f"Ep {ep+1} Size MAE (640 scale): {mae:.5f}")
        if mae < best_mae:
            best_mae = mae
            torch.save(model.state_dict(), "best_size_model_640.pth")
